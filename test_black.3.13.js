...
const FILE = path.join(__dirname, 'black.3.13.jsx');
...
t('APP_VERSION minor=13', /APP_VERSION\s*=\s*\{[\s\S]*?minor:\s*13[,\s]/.test(src));
t('APP_VERSION hotfix=H3.13', /hotfix:\s*['"]H3.13['"]/.test(src));
...
t('Last line says black_3.13', /OTONOM black_3.13/.test(src));
...
t('v3.11: Last line says black_3.13', /OTONOM black_3.13/.test(src));
...
t('v3.12: APP_VERSION minor is 13', /minor:\s*13/.test(src));
t('v3.12: APP_VERSION hotfix is H3.13', /hotfix:\s*'H3\.13'/.test(src));
...
t('v3.12: Last line says black_3.13', /OTONOM black_3\.13/.test(src));

// ═══ v3.13: Audio Bleed + Altyazı + Chart Overlay + Split-Screen + Watermark ═══
t('v3.13: APP_VERSION minor is 13', /minor:\s*13/.test(src));
t('v3.13: APP_VERSION hotfix is H3.13', /hotfix:\s*'H3\.13'/.test(src));
...
t('v3.13: Last line says black_3.13', /OTONOM black_3\.13/.test(src));

// ═══ RESULTS ═══
console.log('═══════════════════════════════════════════════════════════════');
console.log(`  black.3.13.jsx (black_3.13) TEST RESULTS: ${pass} PASS / ${fail} FAIL / ${pass+fail} TOTAL`);
console.log('═══════════════════════════════════════════════════════════════');
...
if (fail > 0) {
  ...
  console.log(`  ✅ TÜM TESTLER BAŞARILI — black.3.13.jsx (black_3.13) ONAYLANDI`);
  process.exit(0);
}
