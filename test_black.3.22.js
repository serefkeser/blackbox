...
const FILE = path.join(__dirname, 'black.3.22.jsx');
...
t('APP_VERSION minor=22', /APP_VERSION\s*=\s*\{[\s\S]*?minor:\s*22[,\s]/.test(src));
t('APP_VERSION hotfix=H3.22', /hotfix:\s*['"]H3.22['"]/.test(src));
...
t('rawSlideSecs has +0 (no buffer)', /spokenText\)\s*\+\s*0;.*v3\.22/.test(src));
t('rawCushion is 0', /rawCushion\s*=\s*0;/.test(src));
t('playAudio has +0 (no buffer)', /scaledExactDur\s*\+\s*0\)/.test(src));
...
t('Last line says black_3.22', /OTONOM black_3.22/.test(src));
...
t('v3.11: Last line says black_3.22', /OTONOM black_3.22/.test(src));
...
t('v3.12: APP_VERSION minor is 22', /minor:\s*22/.test(src));
t('v3.12: APP_VERSION hotfix is H3.22', /hotfix:\s*'H3.22'/.test(src));
...
t('v3.12: Last line says black_3.22', /OTONOM black_3\.22/.test(src));

// ═══ v3.13: Audio Bleed + Altyazı + Chart Overlay + Split-Screen + Watermark ═══
t('v3.13: APP_VERSION minor is 22', /minor:\s*22/.test(src));
t('v3.13: APP_VERSION hotfix is H3.22', /hotfix:\s*'H3.22'/.test(src));
...
t('v3.13: Last line says black_3.22', /OTONOM black_3\.22/.test(src));

// ═══ v3.14: Müzik Seçim Bug Fix ═══
t('v3.14: APP_VERSION minor is 22', /minor:\s*22/.test(src));
t('v3.14: APP_VERSION hotfix is H3.22', /hotfix:\s*'H3.22'/.test(src));
...
t('v3.14: Last line says black_3.22', /OTONOM black_3\.22/.test(src));

// ═══ v3.15: Instagram FPS Fix (VFR→CFR + WebM→MP4) ═══
t('v3.15: APP_VERSION minor is 22', /minor:\s*22/.test(src));
t('v3.15: APP_VERSION hotfix is H3.22', /hotfix:\s*'H3.22'/.test(src));
...
t('v3.15: Last line says black_3.22', /OTONOM black_3\.22/.test(src));

// ═══ v3.16: Export Progress Feedback + Granular ffmpeg Error Handling ═══
t('v3.16: APP_VERSION minor is 22', /minor:\s*22/.test(src));
t('v3.16: APP_VERSION hotfix is H3.22', /hotfix:\s*'H3.22'/.test(src));
...
t('v3.16: Last line says black_3.22', /OTONOM black_3\.22/.test(src));
...
// Version history
t('v3.18: Version history has black_3.18 entry', /black_3.22/.test(src));
...
t('v3.18: Last line says black_3.22', /OTONOM black_3.22/.test(src));
...
t('v3.20: Last line says black_3.22', /OTONOM black_3\.22/.test(src));
...
t('v3.21: Last line says black_3.22', /OTONOM black_3\.22/.test(src));

// ═══ v3.22: Sahneler arası boşluklar kaldırıldı ═══
t('v3.22: playAudio has +0 not +0.3', /scaledExactDur\s*\+\s*0\)/.test(src));
t('v3.22: rawSlideSecs has +0 not +0.3', /spokenText\)\s*\+\s*0;.*v3\.22/.test(src));
t('v3.22: rawCushion is 0', /rawCushion\s*=\s*0;/.test(src));

t('v3.22: no fade-to-black code before outro', !/fadeAlpha.*fi\s*\/\s*fadeFrames/.test(src));
...
t('v3.22: Last line says black_3.22', /OTONOM black_3\.22/.test(src));

console.log(`  black.3.22.jsx (black_3.22) TEST RESULTS: ${pass} PASS / ${fail} FAIL / ${pass+fail} TOTAL`);
console.log('═══════════════════════════════════════════════════════════════');
...
if (fail > 0) {
  ...
  console.log(`  ✅ TÜM TESTLER BAŞARILI — black.3.22.jsx (black_3.22) ONAYLANDI`);
  process.exit(0);
}
