...
const FILE = path.join(__dirname, 'black.3.4.jsx');
...
// 3. APP_VERSION (updated to 3.4)
t('APP_VERSION major=3', /APP_VERSION\s*=\s*\{[\s\S]*?major:\s*3[,\s]/.test(src));
t('APP_VERSION minor=4', /APP_VERSION\s*=\s*\{[\s\S]*?minor:\s*4[,\s]/.test(src));
t('APP_VERSION hotfix=H3.4', /hotfix:\s*['"]H3\.4['"]/.test(src));
...
// 29. Version history
t('Version history has black_3.4', /black_3\.4\s*\(black\.3\.4\)/.test(src));
t('Last line says black_3.4', /OTONOM black_3\.4/.test(src));
...
// Version history has black_3.4
t('Version history has black_3.4 (v2.9 section)', /black_3\.4\s*\(black\.3\.4\)/.test(src));
t('Last line says black_3.4 (v2.9 section)', /OTONOM black_3\.4/.test(src));
...
t('cloud restore uses corsproxy', /corsproxy\.io.*encodeURIComponent/.test(src));
...
// ═══ 44. v3.4: SafeStorage.removeItem + catbox kalıcı + litterbox 72h + çoklu proxy ═══

// SafeStorage.removeItem var (öncesi yoktu)
t('SafeStorage has removeItem', /removeItem:\s*\(key\)/.test(src));

// catbox.moe kalıcı yükleme CORS proxy üzerinden
t('catbox.moe permanent upload via CORS proxy', /catbox\.moe\/user\/api\.php/.test(src));
t('catbox upload uses corsproxy', /corsproxy\.io/.test(src) && /catbox\.moe\/user\/api\.php/.test(src));

// litterbox süresi 72h (1h değil)
t('litterbox time 72h not 1h', /time.*72h/.test(src));
t('litterbox time not 1h', !/time.*'1h'/.test(src));

// loadLocalMusic çoklu CORS proxy fallback
t('Music restore has allorigins fallback', /allorigins\.win.*encodeURIComponent/.test(src));
t('Music restore has direct fetch fallback', /makeUrl\(cu\.url\)/.test(src));

// ═══ RESULTS ═══
console.log('═══════════════════════════════════════════════════════════════');
console.log(`  black.3.4.jsx (black_3.4) TEST RESULTS: ${pass} PASS / ${fail} FAIL / ${pass+fail} TOTAL`);
console.log('═══════════════════════════════════════════════════════════════');
...
if (fail > 0) {
  ...
  console.log(`  ✅ TÜM TESTLER BAŞARILI — black.3.4.jsx (black_3.4) ONAYLANDI`);
  process.exit(0);
}
