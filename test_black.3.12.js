...
const FILE = path.join(__dirname, 'black.3.12.jsx');
...
// 3. APP_VERSION (updated to 3.11)
t('APP_VERSION major=3', /APP_VERSION\s*=\s*\{[\s\S]*?major:\s*3[,\s]/.test(src));
t('APP_VERSION minor=12', /APP_VERSION\s*=\s*\{[\s\S]*?minor:\s*12[,\s]/.test(src));
t('APP_VERSION hotfix=H3.12', /hotfix:\s*['"]H3.12['"]/.test(src));
...
t('Last line says black_3.12', /OTONOM black_3.12/.test(src));
...
t('v3.11: Last line says black_3.12', /OTONOM black_3.12/.test(src));
...
t('v3.12: APP_VERSION minor is 12', /minor:\s*12/.test(src));
t('v3.12: APP_VERSION hotfix is H3.12', /hotfix:\s*'H3\.12'/.test(src));
...
t('v3.12: Last line says black_3.12', /OTONOM black_3\.12/.test(src));

// ═══ RESULTS ═══
console.log('═══════════════════════════════════════════════════════════════');
console.log(`  black.3.12.jsx (black_3.11) TEST RESULTS: ${pass} PASS / ${fail} FAIL / ${pass+fail} TOTAL`);
console.log('═══════════════════════════════════════════════════════════════');
...
if (fail > 0) {
  ...
  console.log(`  ✅ TÜM TESTLER BAŞARILI — black.3.12.jsx (black_3.11) ONAYLANDI`);
  process.exit(0);
}
