...
const FILE = path.join(__dirname, 'black.3.1.jsx');
...
// 3. APP_VERSION (updated to 3.1)
t('APP_VERSION major=2', /APP_VERSION\s*=\s*\{[\s\S]*?major:\s*3[,\s]/.test(src));
t('APP_VERSION minor=1', /APP_VERSION\s*=\s*\{[\s\S]*?minor:\s*1[,\s]/.test(src));
t('APP_VERSION hotfix=H3.1', /hotfix:\s*['"]H3\.1['"]/.test(src));
...
// 29. Version history
t('Version history has black_3.1', /black_3\.1\s*\(black\.3\.1\)/.test(src));
t('Last line says black_3.1', /OTONOM black_3\.1/.test(src));
...
// Version history has black_3.1
t('Version history has black_3.1 (v2.9 section)', /black_3\.1\s*\(black\.3\.1\)/.test(src));
t('Last line says black_3.1 (v2.9 section)', /OTONOM black_3\.1/.test(src));
...
// ═══ RESULTS ═══
console.log('═══════════════════════════════════════════════════════════════');
console.log(`  black.3.1.jsx (black_3.1) TEST RESULTS: ${pass} PASS / ${fail} FAIL / ${pass+fail} TOTAL`);
console.log('═══════════════════════════════════════════════════════════════');
...
if (fail > 0) {
  ...
  console.log(`  ✅ TÜM TESTLER BAŞARILI — black.3.1.jsx (black_3.1) ONAYLANDI`);
  process.exit(0);
}
