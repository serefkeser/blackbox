...
const FILE = path.join(__dirname, 'black.3.16.jsx');
...
t('APP_VERSION minor=16', /APP_VERSION\s*=\s*\{[\s\S]*?minor:\s*16[,\s]/.test(src));
t('APP_VERSION hotfix=H3.16', /hotfix:\s*['"]H3.16['"]/.test(src));
...
t('Line count ~6300', lines.length >= 6000 && lines.length <= 6600, `${lines.length} lines`);
...
t('Last line says black_3.16', /OTONOM black_3.16/.test(src));
...
t('v3.11: Last line says black_3.16', /OTONOM black_3.16/.test(src));
...
t('v3.12: APP_VERSION minor is 16', /minor:\s*16/.test(src));
t('v3.12: APP_VERSION hotfix is H3.16', /hotfix:\s*'H3\.16'/.test(src));
...
t('v3.12: Last line says black_3.16', /OTONOM black_3.16/.test(src));

// ═══ v3.13: Audio Bleed + Altyazı + Chart Overlay + Split-Screen + Watermark ═══
t('v3.13: APP_VERSION minor is 16', /minor:\s*16/.test(src));
t('v3.13: APP_VERSION hotfix is H3.16', /hotfix:\s*'H3\.16'/.test(src));
...
t('v3.13: Last line says black_3.16', /OTONOM black_3.16/.test(src));

// ═══ v3.14: Müzik Seçim Bug Fix ═══
t('v3.14: APP_VERSION minor is 16', /minor:\s*16/.test(src));
t('v3.14: APP_VERSION hotfix is H3.16', /hotfix:\s*'H3\.16'/.test(src));
...
t('v3.14: Last line says black_3.16', /OTONOM black_3\.16/.test(src));

// ═══ v3.15: Instagram FPS Fix (VFR→CFR + WebM→MP4) ═══
t('v3.15: APP_VERSION minor is 16', /minor:\s*16/.test(src));
t('v3.15: APP_VERSION hotfix is H3.16', /hotfix:\s*'H3\.16'/.test(src));
...
t('v3.15: Last line says black_3.16', /OTONOM black_3\.16/.test(src));

// ═══ v3.16: Export Progress Feedback + Granular ffmpeg Error Handling ═══
t('v3.16: APP_VERSION minor is 16', /minor:\s*16/.test(src));
t('v3.16: APP_VERSION hotfix is H3.16', /hotfix:\s*'H3\.16'/.test(src));
...
t('v3.16: convertWebMtoMP4 has try/catch', /try\s*\{[\s\S]*?_loadFFmpeg[\s\S]*?\}\s*catch/.test(src.match(/const convertWebMtoMP4[\s\S]{0,800}/)?.[0] || ''));
...
t('v3.16: Last line says black_3.16', /OTONOM black_3\.16/.test(src));

// ═══ RESULTS ═══
console.log('═══════════════════════════════════════════════════════════════');
console.log(`  black.3.16.jsx (black_3.16) TEST RESULTS: ${pass} PASS / ${fail} FAIL / ${pass+fail} TOTAL`);
console.log('═══════════════════════════════════════════════════════════════');
...
if (fail > 0) {
  ...
  console.log(`  ✅ TÜM TESTLER BAŞARILI — black.3.16.jsx (black_3.16) ONAYLANDI`);
  process.exit(0);
}
