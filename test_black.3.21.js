...
const FILE = path.join(__dirname, 'black.3.21.jsx');
...
t('APP_VERSION minor=21', /APP_VERSION\s*=\s*\{[\s\S]*?minor:\s*21[,\s]/.test(src));
t('APP_VERSION hotfix=H3.21', /hotfix:\s*['"]H3.21['"]/.test(src));
...
t('Last line says black_3.21', /OTONOM black_3.21/.test(src));
...
t('v3.11: Last line says black_3.21', /OTONOM black_3.21/.test(src));
...
t('v3.12: APP_VERSION minor is 21', /minor:\s*21/.test(src));
t('v3.12: APP_VERSION hotfix is H3.21', /hotfix:\s*'H3.21'/.test(src));
...
t('v3.12: Last line says black_3.21', /OTONOM black_3\.21/.test(src));

// ═══ v3.13: Audio Bleed + Altyazı + Chart Overlay + Split-Screen + Watermark ═══
t('v3.13: APP_VERSION minor is 21', /minor:\s*21/.test(src));
t('v3.13: APP_VERSION hotfix is H3.21', /hotfix:\s*'H3.21'/.test(src));
...
t('v3.13: Version history has black_3.13 entry', /black_3.21/.test(src));
t('v3.13: Version history mentions Audio Bleed', /black_3\.13[\s\S]*?Audio Bleed/.test(src));
t('v3.13: Last line says black_3.21', /OTONOM black_3\.21/.test(src));

// ═══ v3.14: Müzik Seçim Bug Fix ═══
t('v3.14: APP_VERSION minor is 21', /minor:\s*21/.test(src));
t('v3.14: APP_VERSION hotfix is H3.21', /hotfix:\s*'H3.21'/.test(src));
...
t('v3.14: Last line says black_3.21', /OTONOM black_3\.21/.test(src));

// ═══ v3.15: Instagram FPS Fix (VFR→CFR + WebM→MP4) ═══
t('v3.15: APP_VERSION minor is 21', /minor:\s*21/.test(src));
t('v3.15: APP_VERSION hotfix is H3.21', /hotfix:\s*'H3.21'/.test(src));
...
t('v3.15: Last line says black_3.21', /OTONOM black_3\.21/.test(src));

// ═══ v3.16: Export Progress Feedback + Granular ffmpeg Error Handling ═══
t('v3.16: APP_VERSION minor is 21', /minor:\s*21/.test(src));
t('v3.16: APP_VERSION hotfix is H3.21', /hotfix:\s*'H3.21'/.test(src));
...
t('v3.16: Last line says black_3.21', /OTONOM black_3\.21/.test(src));
...
// Version history
t('v3.18: Version history has black_3.18 entry', /black_3.21/.test(src));
t('v3.18: Version history mentions Render Resume', /black_3.21[\s\S]*?Render Resume/i.test(src));
t('v3.18: Version history mentions SRT', /black_3.21[\s\S]*?SRT/i.test(src));
t('v3.18: Version history mentions Batch Queue', /black_3.21[\s\S]*?Batch/i.test(src));
t('v3.18: Last line says black_3.21', /OTONOM black_3.21/.test(src));
...
// Version history
t('v3.18: Version history has black_3.21 entry', /black_3\.20/.test(src));
t('v3.18: Last line says black_3.21', /OTONOM black_3\.21/.test(src));

// ═══ v3.20: Hürriyet+Milliyet via gzt.com ═══
t('v3.20: nb-ekonomi slug exists', /slug:\s*['"]nb-ekonomi['"]/.test(src));
t('v3.20: yenibirlik slug exists', /slug:\s*['"]yenibirlik['"]/.test(src));
t('v3.20: no nasil-bir-ekonomi slug', !/slug:\s*['"]nasil-bir-ekonomi['"]/.test(src));
t('v3.20: no yeni-birlik slug (with hyphen)', !/slug:\s*['"]yeni-birlik['"]/.test(src));
t('v3.20: hurriyet exists in NEWSPAPER_DIRECT_CONFIG', /name:\s*['"]Hürriyet['"]/.test(src));
t('v3.20: milliyet exists in NEWSPAPER_DIRECT_CONFIG', /name:\s*['"]Milliyet['"]/.test(src));
t('v3.20: no yeni-mesaj in NEWSPAPER_DIRECT_CONFIG', !/name:\s*['"]Yeni Mesaj['"]/.test(src));
t('v3.20: hurriyet in ALLOWED_GAZETELER', /ALLOWED_GAZETELER\s*=\s*\[[^\]]*Hürriyet/.test(src));
t('v3.20: milliyet in ALLOWED_GAZETELER', /ALLOWED_GAZETELER\s*=\s*\[[^\]]*Milliyet/.test(src));
...
t('v3.20: Version history has black_3.21 entry', /black_3\.20/.test(src));
t('v3.20: Last line says black_3.21', /OTONOM black_3\.21/.test(src));

// ═══ RESULTS ═══
console.log('═══════════════════════════════════════════════════════════════');

// ═══ v3.20: Hürriyet+Milliyet via gzt.com ═══
t('v3.20: Hürriyet type gzt', /name:\s*['"]Hürriyet['"],\s*type:\s*['"]gzt['"]/.test(src));
t('v3.20: Milliyet type gzt', /name:\s*['"]Milliyet['"],\s*type:\s*['"]gzt['"]/.test(src));
t('v3.20: gzt source config exists', /gazetemanset\.gzt\.com/.test(src));
t('v3.20: fetchGazeteManşetleri handles gzt type', /item\.type\s*===\s*['"]gzt['"]/.test(src));
t('v3.20: piri.net in gzt source', /img\.piri\.net/.test(src));
t('v3.20: no gazeteoku type in NEWSPAPER_DIRECT_CONFIG', !/type:\s*['"]gazeteoku['"]/.test(src));

// ═══ v3.21: showSaveFilePicker cross-origin fix + 401 log level ═══
t('v3.21: useFileStreaming has self !== top check', /window\.self\s*!==\s*window\.top/.test(src));
...
t('v3.21: Last line says black_3.21', /OTONOM black_3\.21/.test(src));

console.log(`  black.3.21.jsx (black_3.21) TEST RESULTS: ${pass} PASS / ${fail} FAIL / ${pass+fail} TOTAL`);
console.log('═══════════════════════════════════════════════════════════════');
...
if (fail > 0) {
  ...
  console.log(`  ✅ TÜM TESTLER BAŞARILI — black.3.21.jsx (black_3.21) ONAYLANDI`);
  process.exit(0);
}
