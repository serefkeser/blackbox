...
const FILE = path.join(__dirname, 'black.3.10.jsx');
...
// 3. APP_VERSION (updated to 3.10)
t('APP_VERSION major=3', /APP_VERSION\s*=\s*\{[\s\S]*?major:\s*3[,\s]/.test(src));
t('APP_VERSION minor=10', /APP_VERSION\s*=\s*\{[\s\S]*?minor:\s*10[,\s]/.test(src));
t('APP_VERSION hotfix=H3.10', /hotfix:\s*['"]H3\.10['"]/.test(src));
...
t('Line count ~5970', lines.length >= 5800 && lines.length <= 6300, `${lines.length} lines`);
...
t('Version history has black_3.10', /black_3\.10\s*\(black\.3\.10\)/.test(src));
t('Last line says black_3.10', /OTONOM black_3\.10/.test(src));
...
t('v3.8: Raw video frame-by-frame canvas draw', /drawImageContain\(ctx,\s*rawEl/.test(src));
...
// ═══ v3.10: Siyah ekran fix + grafik kuralı + no-text + fade-out ═══
t('v3.10: drawImageContain uses videoWidth', /videoWidth.*naturalWidth.*img\.width/.test(src));
t('v3.10: drawImageCover uses videoWidth', /videoWidth.*naturalWidth.*img\.width/.test(src));
t('v3.10: drawImageContain has early return for zero dims', /if\s*\(iw\s*<\s*1\s*\|\|\s*ih\s*<\s*1\)\s*return/.test(src));
t('v3.10: No seeking in raw video render (no currentTime assignment)', !/rawEl\.currentTime\s*=/.test(src));
t('v3.10: Prompt has GRAFİK/İNFOGRAFİK KURALI', /GRAF\u0130K\/\u0130NFOGRAF\u0130K KURALI/.test(src));
t('v3.10: Prompt has GÖRSEL YAZI KURALI', /G\u00d6RSEL YAZI KURALI/.test(src));
t('v3.10: generateImage has no words no letters', /no words.*no letters.*no labels.*clean visual/.test(src));
t('v3.10: Fade-to-black before outro', /fade-to-black|fadeAlpha.*rgba\(0,0,0/.test(src));
t('v3.10: Version history has black_3.10', /black_3\.10\s*\(black\.3\.10\)/.test(src));
t('v3.10: Last line says black_3.10', /OTONOM black_3\.10/.test(src));

// ═══ RESULTS ═══
console.log('═══════════════════════════════════════════════════════════════');
console.log(`  black.3.10.jsx (black_3.10) TEST RESULTS: ${pass} PASS / ${fail} FAIL / ${pass+fail} TOTAL`);
console.log('═══════════════════════════════════════════════════════════════');
...
if (fail > 0) {
  ...
  console.log(`  ✅ TÜM TESTLER BAŞARILI — black.3.10.jsx (black_3.10) ONAYLANDI`);
  process.exit(0);
}
