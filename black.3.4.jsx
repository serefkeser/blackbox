...
//   - loadLocalMusic geri yükleme: çoklu CORS proxy fallback (corsproxy.io → allorigins.win → direct)
        ...
        const playAudio = async (audioData, requestedDuration = null, fallbackText = "") => {
          ...
          if (audioData?.wavBuffer && audioCtx) {
            try {
              ...
              source.playbackRate.value = Math.max(scaleFactor, RENDER_CONFIG.SPEECH_RATE); // v3.2: Normal hız (1.0)
            ...
            export default function App() {
              ...
              const uploadMediaToCloud = async (blobOrUrl, fileName = 'video.mp4') => {
                try {
                  ...
                  // Servis 1: catbox.moe (KALICI — sonsuz saklama, 200MB'a kadar) CORS proxy üzerinden
                  try {
                    ...
                    for (const proxy of catProxies) {
                      try {
                        const rCat = await fetch(proxy + encodeURIComponent('https://catbox.moe/user/api.php'), { method: 'POST', body: fdCat });
                        if (rCat.ok) {
                  ...
                  // Servis 3: Yerel Python Sunucu Köprüsü
                  try {
                  ...
                  // Servis 4: litterbox.catbox.moe (72 saat — geçici ama daha uzun süreli)
                  try {
                      ...
                      if (allMusic.length > 0) {
                        ...
                        if (cloudUrls.length > 0) {
                          ...
                          for (const cu of cloudUrls) {
                            let restored = false;
                            for (const makeUrl of restoreProxies) {
                              try {
                                const proxyUrl = makeUrl(cu.url);
                                ...
                                restored = true;
                                break;
                              } catch (ce) { /* try next proxy */ }
                            }
                            if (!restored) addSystemLog(`Geri yükleme başarısız: ${cu.name}`, 'warn');
                          }
                          if (restoredCount > 0) {
              ...
              const handleFolderSelectLegacy = async (e) => {
                ...
                for (const file of audioFiles) {
                  ...
                  await AssetManagerService.saveMusicToLib({ id, name: file.name, data: b64 });
                  savedCount++;
                  // v3.2: catbox.moe CORS proxy üzerinden yükle — kalıcı URL al, localStorage'a kaydet
                  if (!existingCloudIds.has(id)) {
                    try {
                      const blob = _base64ToBlob(b64);
                      const cloudUrl = await uploadMediaToCloud(blob, file.name);
                      if (cloudUrl) {
                        cloudUrls.push({ id, name: file.name, url: cloudUrl });
                        addSystemLog(`☁ ${file.name} buluta yedeklendi`, 'info');
                      } else {
                        addSystemLog(`Bulut yedek atlandı: ${file.name} (IndexedDB'de mevcut, CORS engeli olabilir)`, 'warn');
                      }
                    } catch (ce) { addSystemLog(`Bulut yedek atlandı: ${file.name} (IndexedDB'de mevcut)`, 'warn'); }
                  }
                }
...
// OTONOM black_3.4 — Gemini Canvas uyumlu versiyon
