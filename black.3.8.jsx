...
//   - 30 gazete doğrudan CDN haritası + weserv.nl CORS-free görsel çekme
    ...
    const RenderWorkerService = {
      ...
      renderGuzelSoz: async (jobData, canvasElement, w, h, cx, fontFamily, preferences) => {
        ...
        if (audioData?.wavBuffer) {
          try {
            ...
            source.playbackRate.value = RENDER_CONFIG.SPEECH_RATE; // v3.2: Normal konuşma hızı (1.0)
      ...
      executeRender: async (jobData, canvasElement, preferences) => {
        ...
        let rawSlideSecs = jobData.script.videoSlides.map((s, i) => {
          // v3.8: Raw media sahnesi için TTS süresi değil, medyanın gerçek süresi kullanılır
          if (s._isRawMedia && jobData.script._originalMedia) {
            const rawMedia = jobData.script._originalMedia[s._rawMediaIndex || 0];
            if (rawMedia) {
              // Medya süresini blob'dan hesapla (sync — yaklaşık değer)
              try {
                const rawBlob = _base64ToBlob(rawMedia.data);
                const rawUrl = ObjectURLManager.create(rawBlob);
                // Sync hesaplama yapılamaz, yaklaşık 10sn varsayalım
                // Gerçek süre render sırasında video.duration'dan alınır
                ObjectURLManager.revoke(rawUrl);
                return 10.0; // Placeholder — render sırasında gerçek süre kullanılır
              } catch(e) { return 10.0; }
            }
          }
        ...
        try {
          ...
          for (let i = 0; i < jobData.script.videoSlides.length; i++) {
            ...
            // v3.8: RAW MEDIA sahnesi — orijinal ses/video kesiksiz oynatılır
            if (slide._isRawMedia && jobData.script._originalMedia) {
              ...
              if (!rawMedia) { addSystemLog('Raw medya bulunamad\u0131, atlan\u0131yor.', 'warn'); }
                ...
                addSystemLog(`Raw medya oynat\u0131l\u0131yor: ${isVideo ? 'video' : 'audio'} (kesiksiz)`, 'info');
                // Medyay\u0131 blob URL'e \u00e7evir
                const rawBlob = _base64ToBlob(rawMedia.data);
                const rawUrl = ObjectURLManager.create(rawBlob);
                if (isVideo) {
                  // Video: frame-by-frame canvas'a \u00e7iz, orijinal sesini \u00e7al
                  const rawVideo = document.createElement('video');
                  rawVideo.src = rawUrl; rawVideo.muted = true; rawVideo.playsInline = true;
                  await new Promise((res) => { rawVideo.onloadedmetadata = res; });
                  const rawDur = rawVideo.duration;
                  const rawTotalFrames = Math.max(1, Math.round(rawDur * FPS));
                  // Orijinal ses i\u00e7in audio element
                  const rawAudioEl = document.createElement('video');
                  rawAudioEl.src = rawUrl; rawAudioEl.muted = false;
                  rawAudioEl.play().catch(e => { addSystemLog('Raw ses \u00e7al\u0131namad\u0131: ' + e.message, 'warn'); });
                  rawVideo.play().catch(e => { addSystemLog('Raw video oynat\u0131lamad\u0131: ' + e.message, 'warn'); });
                  for (let frame = 0; frame < rawTotalFrames; frame++) {
                    const elapsedSec = frame / FPS;
                    if (rawVideo.currentTime < elapsedSec) rawVideo.currentTime = elapsedSec;
                    // Video frame'i canvas'a \u00e7iz (contain modunda)
                    ctx.fillStyle = '#000'; ctx.fillRect(0, 0, w, h);
                    RenderWorkerService.drawImageContain(ctx, rawVideo, w, h);
                    // "\u0130\u015eTE KANIT" ba\u015fl\u0131\u011f\u0131
                    if (slide.topText) {
                  ...
                  rawVideo.pause(); rawAudioEl.pause();
                  addSystemLog('Raw video oynat\u0131m\u0131 tamamland\u0131.', 'success');
                } else if (isAudio) {
                  // Audio: clickbait g\u00f6rselini sabit g\u00f6ster, orijinal sesi \u00e7al
                  const rawAudioEl = document.createElement('audio');
                  rawAudioEl.src = rawUrl; rawAudioEl.muted = false;
                  await new Promise((res) => { rawAudioEl.onloadedmetadata = res; });
                  const rawDur = rawAudioEl.duration;
                  const rawTotalFrames = Math.max(1, Math.round(rawDur * FPS));
                  rawAudioEl.play().catch(e => { addSystemLog('Raw ses \u00e7al\u0131namad\u0131: ' + e.message, 'warn'); });
                  for (let frame = 0; frame < rawTotalFrames; frame++) {
                    // Clickbait g\u00f6rselini g\u00f6ster
                    if (tImg) RenderWorkerService.drawImageCover(ctx, tImg, w, h);
                    else { ctx.fillStyle = '#0B0F19'; ctx.fillRect(0, 0, w, h); }
                    // "\u0130\u015eTE KANIT" ba\u015fl\u0131\u011f\u0131
                    if (slide.topText) {
                      ctx.font = `900 ${w > 800 ? 46 : 38}px ${fontFamily}`;
                      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
                  ...
                  rawAudioEl.pause();
                  addSystemLog('Raw ses oynat\u0131m\u0131 tamamland\u0131.', 'success');
                }
                ObjectURLManager.revoke(rawUrl);
              }
              ...
              if (allBasliklar.length >= 1) {
                ...
                try {
                  ...
                  if (this.state.status === 'GENERATING_ASSETS') {
                    ...
                    // v3.2: Müzik seçimi artık if/else dışında — hem normal hem gazete modunda çalışır
                    const allMusic = await AssetManagerService.getAllMusicFromLib();
                    if (allMusic.length > 0) {
    ...
    return this.props.children;
            ...
            export default function App() {
              ...
              const NEWSPAPER_DIRECT_CONFIG = [
                ...
                { name: 'Hürriyet', type: 'aydinlik', slug: 'hurriyet' },
                ...
                { name: 'Milliyet', type: 'aydinlik', slug: 'milliyet' },
                { name: 'Nasıl Bir Ekonomi', type: 'aydinlik', slug: 'nasil-bir-ekonomi' },
                ...
                { name: 'Yeni Birlik', type: 'aydinlik', slug: 'yeni-birlik' },
                { name: 'Yeni Mesaj', type: 'aydinlik', slug: 'yeni-mesaj' },
...
// OTONOM black_3.8 — Gemini Canvas uyumlu versiyon
