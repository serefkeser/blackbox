...
const FILE = path.join(__dirname, 'black.3.5.jsx');
...
// 3. APP_VERSION (updated to 3.5)
t('APP_VERSION major=3', /APP_VERSION\s*=\s*\{[\s\S]*?major:\s*3[,\s]/.test(src));
t('APP_VERSION minor=5', /APP_VERSION\s*=\s*\{[\s\S]*?minor:\s*5[,\s]/.test(src));
t('APP_VERSION hotfix=H3.5', /hotfix:\s*['"]H3\.5['"]/.test(src));
...
t('cloud restore uses corsproxy', /corsproxy\.io\/\?url=.*encodeURIComponent\(u\)/.test(src) && /const\s+proxyUrl\s*=\s*makeUrl\(cu\.url\)/.test(src));
...
t('Last line says black_3.5', /OTONOM black_3\.5/.test(src));
...
// v3.5 version history comments
t('Version history has black_3.5', /black_3\.5\s*\(black\.3\.5\)/.test(src));
t('Last line says black_3.5', /OTONOM black_3\.5/.test(src));

// ═══ RESULTS ═══
console.log('═══════════════════════════════════════════════════════════════');
console.log(`  black.3.5.jsx (black_3.5) TEST RESULTS: ${pass} PASS / ${fail} FAIL / ${pass+fail} TOTAL`);
console.log('═══════════════════════════════════════════════════════════════');
...
if (fail > 0) {
  ...
  console.log(`  ✅ TÜM TESTLER BAŞARILI — black.3.5.jsx (black_3.5) ONAYLANDI`);
  process.exit(0);
}
