...
const FILE = path.join(__dirname, 'black_2.3.jsx');
...
t('APP_VERSION hotfix=H2.3', /hotfix:\s*['"]H2\.3['"]/.test(src));
...
t('No https://localhost in proxies', !lines.some((l,i) => !isComment(l) && /https:\/\/localhost/.test(l)));
...
t('Line count ~4880', lines.length >= 4880 && lines.length <= 5010, `${lines.length} lines`);
...
// ═══ RESULTS ═══
console.log('═══════════════════════════════════════════════════════════════');
console.log(`  black_2.3.jsx TEST RESULTS: ${pass} PASS / ${fail} FAIL / ${pass+fail} TOTAL`);
console.log('═══════════════════════════════════════════════════════════════');
...
if (fail > 0) {
  ...
  console.log(`  ✅ TÜM TESTLER BAŞARILI — black_2.3.jsx ONAYLANDI`);
  process.exit(0);
}
