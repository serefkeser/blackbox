...
const FILE = path.join(__dirname, 'black.3.17.jsx');
...
t('APP_VERSION minor=17', /APP_VERSION\s*=\s*\{[\s\S]*?minor:\s*17[,\s]/.test(src));
t('APP_VERSION hotfix=H3.17', /hotfix:\s*['"]H3.17['"]/.test(src));
...
t('Line count ~6300', lines.length >= 6300 && lines.length <= 6600, `${lines.length} lines`);
...
t('Last line says black_3.17', /OTONOM black_3.17/.test(src));
...
t('v3.11: Last line says black_3.17', /OTONOM black_3.17/.test(src));
...
t('v3.12: APP_VERSION minor is 17', /minor:\s*17/.test(src));
t('v3.12: APP_VERSION hotfix is H3.17', /hotfix:\s*'H3\.17'/.test(src));
...
t('v3.12: Last line says black_3.17', /OTONOM black_3.17/.test(src));

// ═══ v3.13: Audio Bleed + Altyazı + Chart Overlay + Split-Screen + Watermark ═══
t('v3.13: APP_VERSION minor is 17', /minor:\s*17/.test(src));
t('v3.13: APP_VERSION hotfix is H3.17', /hotfix:\s*'H3\.17'/.test(src));
...
t('v3.13: Last line says black_3.17', /OTONOM black_3.17/.test(src));

// ═══ v3.14: Müzik Seçim Bug Fix ═══
t('v3.14: APP_VERSION minor is 17', /minor:\s*17/.test(src));
t('v3.14: APP_VERSION hotfix is H3.17', /hotfix:\s*'H3\.17'/.test(src));
...
t('v3.14: Last line says black_3.17', /OTONOM black_3\.17/.test(src));

// ═══ v3.15: Instagram FPS Fix (VFR→CFR + WebM→MP4) ═══
t('v3.15: APP_VERSION minor is 17', /minor:\s*17/.test(src));
t('v3.15: APP_VERSION hotfix is H3.17', /hotfix:\s*'H3\.17'/.test(src));
...
t('v3.15: Last line says black_3.17', /OTONOM black_3\.17/.test(src));

// ═══ v3.16: Export Progress Feedback + Granular ffmpeg Error Handling ═══
t('v3.16: APP_VERSION minor is 17', /minor:\s*17/.test(src));
t('v3.16: APP_VERSION hotfix is H3.17', /hotfix:\s*'H3\.17'/.test(src));
...
t('v3.16: Last line says black_3.17', /OTONOM black_3\.17/.test(src));

// ═══ v3.17: Export Presets + Web Speech API + Preview/Low-Res Mode ═══

// EXPORT_PRESETS
t('v3.17: EXPORT_PRESETS defined', /const\s+EXPORT_PRESETS\s*=/.test(src));
t('v3.17: EXPORT_PRESETS has instagram_reels', /instagram_reels/.test(src));
t('v3.17: EXPORT_PRESETS has tiktok', /tiktok/.test(src));
t('v3.17: EXPORT_PRESETS has youtube_shorts', /youtube_shorts/.test(src));
t('v3.17: EXPORT_PRESETS has twitter', /twitter:\s*\{/.test(src));
t('v3.17: EXPORT_PRESETS has facebook', /facebook:\s*\{/.test(src));
t('v3.17: EXPORT_PRESETS has linkedin', /linkedin:\s*\{/.test(src));
t('v3.17: EXPORT_PRESETS has custom', /custom:\s*\{/.test(src));
t('v3.17: instagram_reels is 9:16 1K mp4', /instagram_reels:\s*\{[^}]*aspectRatio:\s*['"]9:16['"][^}]*resolution:\s*['"]1K['"][^}]*videoFormat:\s*['"]mp4['"]/.test(src));
t('v3.17: youtube_shorts is 9:16 2K mp4', /youtube_shorts:\s*\{[^}]*aspectRatio:\s*['"]9:16['"][^}]*resolution:\s*['"]2K['"]/.test(src));
t('v3.17: twitter is 16:9 1K mp4', /twitter:\s*\{[^}]*aspectRatio:\s*['"]16:9['"][^}]*resolution:\s*['"]1K['"]/.test(src));
t('v3.17: EXPORT_PRESETS has bitrate field', /bitrate:\s*4_000_000/.test(src));
t('v3.17: EXPORT_PRESETS has duration field', /duration:\s*['"]30['"]/.test(src));

// CANVAS_DIMENSIONS
t('v3.17: CANVAS_DIMENSIONS defined', /const\s+CANVAS_DIMENSIONS\s*=/.test(src));
t('v3.17: CANVAS_DIMENSIONS has 9:16', /['"]9:16['"]:\s*\{/.test(src));
t('v3.17: CANVAS_DIMENSIONS has 16:9', /['"]16:9['"]:\s*\{/.test(src));
t('v3.17: CANVAS_DIMENSIONS has 1:1', /['"]1:1['"]:\s*\{/.test(src));
t('v3.17: CANVAS_DIMENSIONS 9:16 1K is 720x1280', /['"]9:16['"]:\s*\{[^}]*['"]1K['"]:\s*\[720,\s*1280\]/.test(src));
t('v3.17: CANVAS_DIMENSIONS 9:16 2K is 1080x1920', /['"]9:16['"]:\s*\{[^}]*['"]2K['"]:\s*\[1080,\s*1920\]/.test(src));
t('v3.17: CANVAS_DIMENSIONS 16:9 1K is 1280x720', /['"]16:9['"]:\s*\{[^}]*['"]1K['"]:\s*\[1280,\s*720\]/.test(src));

// Web Speech API
t('v3.17: generateVoiceOverWithWebSpeech defined', /generateVoiceOverWithWebSpeech\s*\(/.test(src));
t('v3.17: generateVoiceOverWithWebSpeech is static async', /static\s+async\s+generateVoiceOverWithWebSpeech/.test(src));
t('v3.17: generateVoiceOverWithWebSpeech uses SpeechSynthesisUtterance', /new\s+SpeechSynthesisUtterance/.test(src));
t('v3.17: generateVoiceOverWithWebSpeech uses speechSynthesis.speak', /speechSynthesis\.speak/.test(src));
t('v3.17: generateVoiceOverWithWebSpeech has _webSpeech flag', /_webSpeech:\s*true/.test(src));
t('v3.17: generateAudio has ttsEngine param', /generateAudio\s*\(\s*text,\s*voice,\s*ttsEngine/.test(src));
t('v3.17: generateAudio checks webspeech engine', /ttsEngine\s*===\s*['"]webspeech['"]/.test(src));
t('v3.17: generateAudio falls back to Gemini on Web Speech fail', /Web Speech başarısız.*Gemini TTS/.test(src));
t('v3.17: Gemini TTS retry falls back to Web Speech', /Web Speech fallback/.test(src));

// Preview/Low-Res mode
t('v3.17: PREVIEW_FPS is 15', /PREVIEW_FPS:\s*15/.test(src));
t('v3.17: PREVIEW_BITRATE is 1_000_000', /PREVIEW_BITRATE:\s*1_000_000/.test(src));
t('v3.17: PREVIEW_SCALE is 0.5', /PREVIEW_SCALE:\s*0\.5/.test(src));
t('v3.17: Render FPS uses previewMode check', /previewMode\s*===\s*true\s*\)\s*\?\s*RENDER_CONFIG\.PREVIEW_FPS/.test(src));
t('v3.17: Render bitrate uses previewMode check', /previewMode\s*===\s*true\s*\)\s*\?\s*RENDER_CONFIG\.PREVIEW_BITRATE/.test(src));
t('v3.17: Canvas dims use previewMode scale', /isPreviewMode.*PREVIEW_SCALE/.test(src));
t('v3.17: Canvas dims from CANVAS_DIMENSIONS', /CANVAS_DIMENSIONS\[aspectRatio\]/.test(src));
t('v3.17: GuzelSoz bitrate uses previewMode', /_guzelSozBitrate.*previewMode.*PREVIEW_BITRATE/.test(src));
t('v3.17: Render bitrate uses EXPORT_PRESETS', /EXPORT_PRESETS\[.*exportPreset\]\?\.bitrate/.test(src));

// UI: Export Preset buttons
t('v3.17: UI has EXPORT_PRESETS buttons', /Object\.entries\(EXPORT_PRESETS\)\.map/.test(src));
t('v3.17: UI exportPreset in config default', /exportPreset:\s*['"]custom['"]/.test(src));
t('v3.17: UI preset click sets config', /setConfig.*exportPreset:\s*key/.test(src));
t('v3.17: UI preset click sets aspectRatio', /setConfig.*aspectRatio:\s*preset\.aspectRatio/.test(src));

// UI: TTS engine toggle
t('v3.17: UI has ttsEngine in config default', /ttsEngine:\s*['"]gemini['"]/.test(src));
t('v3.17: UI has Gemini TTS button', /ttsEngine:\s*['"]gemini['"]/.test(src));
t('v3.17: UI has Web Speech button', /ttsEngine:\s*['"]webspeech['"]/.test(src));

// UI: Preview mode toggle
t('v3.17: UI has previewMode in config default', /previewMode:\s*false/.test(src));
t('v3.17: UI has previewMode toggle button', /previewMode:\s*!config\.previewMode/.test(src));
t('v3.17: Preview mode skips AI image generation', /isPreview\s*\?\s*Promise\.resolve\(null\)/.test(src));
t('v3.17: Preview mode skips TTS generation', /isPreview\s*\?\s*Promise\.resolve\(null\).*generateAudio/.test(src));

// Version history
t('v3.17: Version history has black_3.17 entry', /black_3\.17/.test(src));
t('v3.17: Version history mentions Export Presets', /black_3\.17[\s\S]*?Export Presets/.test(src));
t('v3.17: Version history mentions Web Speech', /black_3\.17[\s\S]*?Web Speech/.test(src));
t('v3.17: Version history mentions Preview', /black_3\.17[\s\S]*?Preview/.test(src));
t('v3.17: Last line says black_3.17', /OTONOM black_3\.17/.test(src));

// ═══ RESULTS ═══
console.log('═══════════════════════════════════════════════════════════════');
console.log(`  black.3.17.jsx (black_3.17) TEST RESULTS: ${pass} PASS / ${fail} FAIL / ${pass+fail} TOTAL`);
console.log('═══════════════════════════════════════════════════════════════');
...
if (fail > 0) {
  ...
  console.log(`  ✅ TÜM TESTLER BAŞARILI — black.3.17.jsx (black_3.17) ONAYLANDI`);
  process.exit(0);
}
