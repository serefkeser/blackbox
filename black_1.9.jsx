...
// black_1.9: Local proxy öncelikli (HTTPS port 3457 — Canvas mixed content çözümü)
// Local proxy pre-parsed JSON döndürür — DOMParser'a gerek yok
// Public proxy'ler yedek (çoğu zaman ölü)
const GAZETE_PROXY_ENDPOINTS = {
  gazeteoku: 'https://localhost:3457/gazeteoku',
  aydinlik: 'https://localhost:3457/aydinlik',
  gzt: 'https://localhost:3457/gzt',
  yenimesaj: 'https://localhost:3457/yenimesaj',
};
const CORS_PROXIES = [
  { url: (u) => `https://localhost:3457/proxy?url=${encodeURIComponent(u)}`, json: false, local: true },
  { url: (u) => `http://localhost:3456/proxy?url=${encodeURIComponent(u)}`, json: false, local: true },
...
const apiKey = "";
// ⚠️ GÜVENLİK UYARISI: Aşağıdaki API key'ler hardcoded olarak duruyor.
// Production ortamında bu key'leri environment variable olarak yükleyin:
//   const NVIDIA_API_KEY = import.meta.env?.VITE_NVIDIA_API_KEY || "";
//   const GROQ_API_KEY = import.meta.env?.VITE_GROQ_API_KEY || "";
// Bu dosyayı public repo'ya commit etmeyin — key'ler sızar!
...
class LogicEngineService {
  ...
  static async _analyzeIddia(inputData, inputType, config) {
    ...
    var url = `https://generativelanguage.googleapis.com/v1beta/models/${AI_CONFIG.GEMINI_MODEL}:generateContent?key=${apiKey}`;
  ...
  static async _buildGuzelSozScript(inputData, inputType, config) {
    ...
    if (typeof inputData === 'string') {
      ...
      if (videoFile) {
        ...
        quoteText = await ocrWithFallback(b64Data, imgType, apiKey, 'Görsel OCR');
  ...
  render() {
    ...
    return this.props.children;
            ...
            export default function App() {
              ...
              const handleCustomSceneImagesUpload = async (e) => { const files = Array.from(e.target ? e.target.files : e); if (!files.length) return; const availableSlots = 999 - (studioMedia.customSceneImages?.length || 0); const filesToProcess = files.slice(0, availableSlots); const newB64s = []; for (let file of filesToProcess) { if (file.type.startsWith('image/')) { const b64 = await NetworkUtils.compressImage(file); newB64s.push(b64); } } const updatedImages = [...(studioMedia.customSceneImages || []), ...newB64s].slice(0, 5); for (let i = 0; i < updatedImages.length; i++) await AssetManagerService.saveMedia("CUSTOM_SCENE_IMG_" + i, updatedImages[i]); setStudioMedia(s => ({ ...s, customSceneImages: updatedImages })); const newMediaFiles = newB64s.map((b64, i) => ({ name: `SabitGorsel_${Date.now()}_${i}.jpg`, type: 'image/jpeg', data: b64 })); if (newMediaFiles.length > 0) setUiState(prev => ({ ...prev, selectedMediaFiles: [...prev.selectedMediaFiles, ...newMediaFiles] })); if (e.target) e.target.value = null; };
              ...
              const fetchGazeteManşetleri = async () => {
                ...
                try {
                  ...
                  if (items.length === 0) throw new Error(`${sourceConfig.label} manşetleri yüklenemedi. Çözüm: terminalde "node gazete-proxy.js" çalıştırın (HTTPS port 3457).`);

                  setGazeteItems(items);
                  if (items.length > 0) addSystemLog(`${sourceConfig.label}: ${items.length} gazete manşeti yüklendi.`, 'success');
                } catch (e) {
              ...
              const parseGazeteHtml = (html, sourceConfig) => {
                ...
                allImgs.forEach(img => {
                    ...
                    if (name.length > 2 && finalSrc.length > 10 && !finalSrc.includes('blank.png') && !finalSrc.includes('logo') && !finalSrc.includes('icon') && !finalSrc.includes('banner') && !finalSrc.includes('avatar') && !seen.has(name)) {
                      seen.add(name);
              ...
              return (
                ...
                BLACKBOX black_2.1 <span className="mx-1 text-white">•</span> One-Page
              </p>
              </div>
              </div>
                ...
                {activeTab === 'gazete' && (
                    ...
                    {/* Tarih seçici */}
                    <div className="flex items-center gap-1.5 bg-slate-800/50 border border-slate-700 rounded-lg px-2 py-1">
                    ...
                    className="bg-transparent text-slate-300 text-[10px] font-bold border-none outline-none cursor-pointer"
                    />
                  </div>
...
// OTONOM black_2.1 — Gemini Canvas uyumlu versiyon
