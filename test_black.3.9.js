...
const FILE = path.join(__dirname, 'black.3.9.jsx');
...
t('APP_VERSION hotfix=H3.9', /hotfix:\s*['"]H3\.9['"]/.test(src));
...
t('Line count ~5970', lines.length >= 5800 && lines.length <= 6200, `${lines.length} lines`);
...
t('Version history has black_3.9', /black_3\.9\s*\(black\.3\.9\)/.test(src));
t('Last line says black_3.9', /OTONOM black_3\.9/.test(src));
...
t('v3.9: Version history has black_3.9', /black_3\.9\s*\(black\.3\.9\)/.test(src));
t('v3.9: Last line says black_3.9', /OTONOM black_3\.9/.test(src));

// ═══ RESULTS ═══
console.log('═══════════════════════════════════════════════════════════════');
console.log(`  black.3.9.jsx (black_3.9) TEST RESULTS: ${pass} PASS / ${fail} FAIL / ${pass+fail} TOTAL`);
console.log('═══════════════════════════════════════════════════════════════');
...
if (fail > 0) {
  ...
  console.log(`  ✅ TÜM TESTLER BAŞARILI — black.3.9.jsx (black_3.9) ONAYLANDI`);
  process.exit(0);
}
