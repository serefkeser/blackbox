...
const FILE = path.join(__dirname, 'black.3.19.jsx');
...
t('APP_VERSION minor=19', /APP_VERSION\s*=\s*\{[\s\S]*?minor:\s*19[,\s]/.test(src));
t('APP_VERSION hotfix=H3.19', /hotfix:\s*['"]H3.19['"]/.test(src));
...
t('Last line says black_3.19', /OTONOM black_3.19/.test(src));
...
t('v3.11: Last line says black_3.19', /OTONOM black_3.19/.test(src));
...
t('v3.12: APP_VERSION minor is 19', /minor:\s*19/.test(src));
t('v3.12: APP_VERSION hotfix is H3.19', /hotfix:\s*'H3\.19'/.test(src));
...
t('v3.12: Last line says black_3.19', /OTONOM black_3\.19/.test(src));

// ═══ v3.13: Audio Bleed + Altyazı + Chart Overlay + Split-Screen + Watermark ═══
t('v3.13: APP_VERSION minor is 19', /minor:\s*19/.test(src));
t('v3.13: APP_VERSION hotfix is H3.19', /hotfix:\s*'H3\.19'/.test(src));
...
t('v3.13: Last line says black_3.19', /OTONOM black_3\.19/.test(src));

// ═══ v3.14: Müzik Seçim Bug Fix ═══
t('v3.14: APP_VERSION minor is 19', /minor:\s*19/.test(src));
t('v3.14: APP_VERSION hotfix is H3.19', /hotfix:\s*'H3\.19'/.test(src));
...
t('v3.14: Last line says black_3.19', /OTONOM black_3\.19/.test(src));

// ═══ v3.15: Instagram FPS Fix (VFR→CFR + WebM→MP4) ═══
t('v3.15: APP_VERSION minor is 19', /minor:\s*19/.test(src));
t('v3.15: APP_VERSION hotfix is H3.19', /hotfix:\s*'H3\.19'/.test(src));
...
t('v3.15: Last line says black_3.19', /OTONOM black_3\.19/.test(src));

// ═══ v3.16: Export Progress Feedback + Granular ffmpeg Error Handling ═══
t('v3.16: APP_VERSION minor is 19', /minor:\s*19/.test(src));
t('v3.16: APP_VERSION hotfix is H3.19', /hotfix:\s*'H3\.19'/.test(src));
...
t('v3.16: Last line says black_3.19', /OTONOM black_3\.19/.test(src));
...
// Version history
t('v3.18: Version history has black_3.18 entry', /black_3.19/.test(src));
t('v3.18: Version history mentions Render Resume', /black_3.19[\s\S]*?Render Resume/i.test(src));
t('v3.18: Version history mentions SRT', /black_3.19[\s\S]*?SRT/i.test(src));
t('v3.18: Version history mentions Batch Queue', /black_3.19[\s\S]*?Batch/i.test(src));
t('v3.18: Last line says black_3.19', /OTONOM black_3.19/.test(src));
...
// Version history
t('v3.18: Version history has black_3.19 entry', /black_3\.19/.test(src));
t('v3.18: Last line says black_3.19', /OTONOM black_3\.19/.test(src));

// ═══ v3.19: Gazete CDN Slug Fix ═══
t('v3.19: nb-ekonomi slug exists', /slug:\s*['"]nb-ekonomi['"]/.test(src));
t('v3.19: yenibirlik slug exists', /slug:\s*['"]yenibirlik['"]/.test(src));
t('v3.19: no nasil-bir-ekonomi slug', !/slug:\s*['"]nasil-bir-ekonomi['"]/.test(src));
t('v3.19: no yeni-birlik slug (with hyphen)', !/slug:\s*['"]yeni-birlik['"]/.test(src));
t('v3.19: no hurriyet in NEWSPAPER_DIRECT_CONFIG', !/name:\s*['"]Hürriyet['"]/.test(src));
t('v3.19: no milliyet in NEWSPAPER_DIRECT_CONFIG', !/name:\s*['"]Milliyet['"]/.test(src));
t('v3.19: no yeni-mesaj in NEWSPAPER_DIRECT_CONFIG', !/name:\s*['"]Yeni Mesaj['"]/.test(src));
t('v3.19: no hurriyet in ALLOWED_GAZETELER', !/ALLOWED_GAZETELER\s*=\s*\[[^\]]*Hürriyet/.test(src));
t('v3.19: no milliyet in ALLOWED_GAZETELER', !/ALLOWED_GAZETELER\s*=\s*\[[^\]]*Milliyet/.test(src));
t('v3.19: no yeni-mesaj in ALLOWED_GAZETELER', !/ALLOWED_GAZETELER\s*=\s*\[[^\]]*Yeni Mesaj/.test(src));
t('v3.19: 27 gazete in config', (src.match(/name:\s*['"][^'"]+['"],\s*type:/g) || []).length === 27);
t('v3.19: Version history has black_3.19 entry', /black_3\.19/.test(src));
t('v3.19: Last line says black_3.19', /OTONOM black_3\.19/.test(src));

// ═══ RESULTS ═══
console.log('═══════════════════════════════════════════════════════════════');
console.log(`  black.3.19.jsx (black_3.19) TEST RESULTS: ${pass} PASS / ${fail} FAIL / ${pass+fail} TOTAL`);
console.log('═══════════════════════════════════════════════════════════════');
...
if (fail > 0) {
  ...
  console.log(`  ✅ TÜM TESTLER BAŞARILI — black.3.19.jsx (black_3.19) ONAYLANDI`);
  process.exit(0);
}
